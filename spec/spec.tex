...
Figure \ref{fig:interface} shows a general design for the interface, highlighting important regions, particularly:
...
\noindent though this does not capture 

\section{Applying Modifications}
\label{sec:applymod}

The process for applying modifications is very similar to that of applying definitions. 
...
\subsection{Modifiers for Subprograms}
\label{subsec:mods-graphs}
\begin{tabularx}{\textwidth}{l l l X}
$\mu_{G}$ & $::=$ & $\mathsf{local}$ & Produces a local definition from this program graph. This modifier is used in conjunction with the $\mathsf{handle}$, $\mathsf{each}$ and $\mathsf{filter}$ modifiers to apply more complex behaviors.\\
\end{tabularx}
